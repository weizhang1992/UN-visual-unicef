...
def upload():
    if request.method == 'GET':
        ...
        # reading the data
#         folder = 'OPSCEN Brief 2015/'
        
        try:
            f = open(os.path.join(UPLOAD_FOLDER, fname),'rb')
            document = Document(f)
        ...
        cats = pd.read_csv('app/Preprocess/data/news_stories_plus_LDA.csv', usecols=['row_index','link', 'title', '250topics_NER'], index_col=['row_index'])
